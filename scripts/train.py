...
def main(cfg):
    ...
    trainer = pl.Trainer(
        ...
        num_sanity_val_steps=0, # Sanity check runs n batches of val before starting the training routine. This catches any bugs in your validation without having to wait for the first validation check. The Trainer uses 2 steps by default. Turn it off or modify it here. more visit https://lightning.ai/docs/pytorch/latest/common/trainer.html?highlight=num_sanity_val_steps#num-sanity-val-steps
    )
...
if __name__ == '__main__':
    ...
    projects = ['Co-creation Space Assembly']
    parser.add_argument('--project_name', type=str, default=projects[0], help='the job name of wandb.ai')

    args = parser.parse_args()

    setproctitle(os.path.basename(args.cfg_file)[:-3])
