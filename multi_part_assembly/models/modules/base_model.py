...
class BaseModel(pl.LightningModule):
    ...
    def _calc_loss(self, out_dict, data_dict, C = 1.0):
        ...
        coll_loss = collision_loss(
            ...
            pred_rot,
            valids,
            C=self.cfg.loss.collision_loss_C
        )
    ...
    def loss_function(self, data_dict, optimizer_idx):
        ...
        # loss_dict['balance_loss'] = torch.tensor(loss_dict['balance_loss']).unsqueeze(1).expand(-1, self.cfg.exp.batch_size)
        # take away `balance_loss`
        balance_loss = None
        if 'balance_loss' in loss_dict.keys():
            balance_loss = loss_dict['balance_loss'][0]  # balance_loss 是一个batch中计算一次，所以只有一项；并且 balance_loss并不参与后面的MoN Loss
            loss_dict.pop('balance_loss')
        ...
        if balance_loss is not None:
            ...
            loss_dict['loss'] += balance_loss * eval(f'self.cfg.loss.balance_loss_w')  
