...
from multi_part_assembly.utils.wx_transformer_utilities.GroupLinearLayer import GroupLinearLayer
import math
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, x, mask = None, num_layers = None):
        ...
        Args:
            x: [B, N, C]
            mask: [B, N], True for valid, False for padded
        ...
        if not self.functional:
            ...
            for i in range(self.num_layers):
                if self.share_parameters:
                    x, memory = self.enc(x, pad_masks, memory = memory)
                else:
                    x, memory = self.layers[i](x, pad_masks, memory = memory)
