...
def save_colored_pc(pc, colors, file):
    ...
    Args:
        pc (np.ndarray): [N, 3]
        colors (np.ndarray): [N, 3] or [N, 4]
        file (str): .ply
    """
    if colors.shape[1] == 3:
        # If colors are RGB, add an alpha channel with value 1.
        colors = np.concatenate([colors, np.ones((colors.shape[0], 1))], axis=1)
    pcd_df = pd.DataFrame(data=np.concatenate([pc, colors], axis=1), columns=['x', 'y', 'z', 'red', 'green', 'blue', 'alpha'])
    pcd_cloud = PyntCloud(pcd_df)
    pcd_cloud.to_file(file)
